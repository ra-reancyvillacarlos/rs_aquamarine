    ...
    public partial class m_assembleditem : Form
    {
        ...
        private void btn_itemupd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                frm.txt_itemdesc.Text = dgv_itemlist["dgvi_itemdesc", r].Value.ToString(); //dgv_itemlist.SelectedRows[0].Cells[1].Value.ToString();
        ...
        private void bgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                for (rCnt = 1; rCnt <= rw; rCnt++)
                {
                    ...
                    {
                        //assembled item
                        ai_name = Convert.ToString((range.Cells[rCnt, 1] as Excel.Range).Value2);
                        ...
                        part_no = Convert.ToString((range.Cells[rCnt, 6] as Excel.Range).Value2);
                        name = Convert.ToString((range.Cells[rCnt, 7] as Excel.Range).Value2).Trim();
                        cat = Convert.ToString((range.Cells[rCnt, 8] as Excel.Range).Value2);
                        brd = Convert.ToString((range.Cells[rCnt, 9] as Excel.Range).Value2);
                        reqLot = "Y";
                        ...
                        if(String.IsNullOrEmpty(ai_name) == false)
                        {
                            ...
                            val = "" + db.str_E(ai_code) + ", " + db.str_E(ai_name) + ", '" + stk_item + "', '" + typ + "', '" + unit_cost + "', '" + regsell + "', '" + gp + "', '" + reorder + "', '" + max + "', '" + rack + "', '" + reqLot + "', '" + brd + "', 'MENU', '014', '014', '" + sc + "', 'Y', '0.00','0.00','" + fcp + "','" + ave_cost + "', '" + ccode + "'";
                        ...
                        if (String.IsNullOrEmpty(part_no) == false && db.isExists("items", "part_no= " + db.str_E(part_no) ) == false)
                        {
