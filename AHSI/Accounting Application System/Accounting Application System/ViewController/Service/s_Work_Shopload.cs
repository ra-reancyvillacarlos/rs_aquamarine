...
{
    public partial class s_Work_Shopload : Form
    {
        ...
        public s_Work_Shopload()
        {
            InitializeComponent();
            init_load();

        }
        public s_Work_Shopload(call_history frm, String pk)
        {
            InitializeComponent();
            init_load();
         ...
         public s_Work_Shopload(Boolean cashier)
        {
            InitializeComponent();
            isCashier = cashier;
            init_load();            
        ...
        public s_Work_Shopload(int count)
        {
            ...
            public DataTable get_solist()
            {
                ...
                //}
                if (cbo_brand.SelectedIndex != -1)
                {
                    WHERE += " o.branch='" + cbo_brand.SelectedValue.ToString() + "' AND ";
                }


                DateTime dt_dtime = DateTime.Parse(DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd") + " " + dtp_time.Value.ToString("HH:mm:ss tt"));
                //COALESCE(o.pending,'')='Y' AND '" + dt_dtime.ToString("HH:mm") + "'>=o.t_time AND '" + dt_dtime.ToString("yyyy-MM-dd") + "'>=o.t_date
                return db.QueryBySQLCode("SELECT o.out_code,out.out_desc,o.ord_code,m.d_name as customer,o.payment,o.cancel,o.net_amnt,o.ord_date,o.reference,o.car_plate,o.car_vin_num,o.t_date,o.t_time,o.t_date2,o.tax_amnt,o.pending,o.amnt_due,o.disc_amnt, o.user_id,o.loc,o.user_id2,o.rep_code, o.promise_date,o.promise_time, o.cashier,o.gross_amnt,o.ord_amnt,o.pay_code,o.debt_code as cust_no FROM rssys.orhdr o LEFT JOIN  rssys.whouse w ON w.whs_code=o.loc LEFT JOIN  rssys.m06 m ON m.d_code=o.debt_code LEFT JOIN rssys.outlet out ON o.out_code=out.out_code WHERE " + WHERE + " COALESCE(o.pending,'')='Y' AND (('" + dt_dtime.ToString("HH:mm") + "'>=o.t_time AND '" + dt_dtime.ToString("yyyy-MM-dd") + "'=o.t_date) OR '" + dt_dtime.ToString("yyyy-MM-dd") + "'>o.t_date) ORDER BY o.ord_date DESC,o.t_date DESC, o.t_time DESC");
                //m06
            }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (cbo_custumer1.SelectedIndex != -1)
            {
                //MessageBox.Show("Please Select Customer.");
                //cbo_custumer1.DroppedDown = true;
                WHERE += " AND o.debt_code='" + cbo_custumer1.SelectedValue.ToString() + "' ";
            }
            if (cbo_branch.SelectedIndex != -1)
            {
                WHERE += " AND o.branch='" + cbo_branch.SelectedValue.ToString() + "'  ";
            }
            if (!String.IsNullOrEmpty(txt_roreference.Text))
            {
                WHERE += " AND o.reference LIKE '%" + txt_roreference.Text + "%' ";
            }
            if (!String.IsNullOrEmpty(txt_plateno.Text))
            {
                WHERE += " AND o.car_plate LIKE '%" + txt_plateno.Text + "%' ";
            }
            if (!String.IsNullOrEmpty(txt_vinno.Text))
            {
                WHERE += " AND o.car_vin_num LIKE '%" + txt_vinno.Text + "%' ";
            }
            if (!String.IsNullOrEmpty(txt_transcode.Text))
            {
                WHERE += " AND o.ord_code LIKE '%" + txt_transcode.Text + "%' ";
            }


            DateTime dt_dtime = DateTime.Parse(DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd") + " " + dtp_time.Value.ToString("HH:mm:ss tt"));
            //COALESCE(o.pending,'')='Y' AND '" + dt_dtime.ToString("HH:mm") + "'>=o.t_time AND '" + dt_dtime.ToString("yyyy-MM-dd") + "'>=o.t_date
            DataTable dt = db.QueryBySQLCode("SELECT o.out_code,out.out_desc,o.ord_code,m.d_name as customer,o.payment,o.cancel,o.net_amnt,o.ord_date,o.t_date,o.reference,o.car_plate,o.car_vin_num,o.t_time,o.t_date2,o.tax_amnt,o.pending,o.amnt_due,o.disc_amnt, o.user_id,o.loc,o.user_id2,o.rep_code, o.promise_date,o.promise_time,o.cashier,o.gross_amnt,o.ord_amnt,o.pay_code,o.debt_code as cust_no FROM rssys.orhdr o LEFT JOIN rssys.whouse w ON w.whs_code=o.loc LEFT JOIN  rssys.m06 m ON m.d_code=o.debt_code LEFT JOIN rssys.outlet out ON o.out_code=out.out_code WHERE 1=1 " + WHERE + " AND COALESCE(o.pending,'')='Y' AND (('" + dt_dtime.ToString("HH:mm") + "'>=o.t_time AND '" + dt_dtime.ToString("yyyy-MM-dd") + "'=o.t_date) OR '" + dt_dtime.ToString("yyyy-MM-dd") + "'>o.t_date) ORDER BY o.ord_date DESC,o.t_date DESC, o.t_time DESC");
