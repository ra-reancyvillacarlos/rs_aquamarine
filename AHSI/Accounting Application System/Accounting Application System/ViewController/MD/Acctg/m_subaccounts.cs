...
{
    public partial class m_subaccounts : Form
    {
        Boolean seltbp = false;
        Boolean isnew = false;

        public m_subaccounts()
        {
            GlobalClass gc = new GlobalClass();

            InitializeComponent();

            gc.load_m01(cbo_mag_code);
            ...
            DataTable dt2 = db.QueryBySQLCode("SELECT a.*, b.* FROM rssys.x06 a LEFT JOIN rssys.x05 b ON a.mod_id = b.mod_id  WHERE a.grp_id = '" + grp_id + "' AND a.mod_id='M0102' ORDER BY b.pla, b.mod_id");
        ...
        private void m_subaccounts_Load(object sender, EventArgs e)
        {
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    code = txt_code.Text;
                    col = "cmp_code, cmp_desc, mag_code";
                    val = "'" + code + "', '" + cmp_desc + "', '" + mag_code + "'";

                    if (db.InsertOnTable("m02", col, val))
                    {
                        //db.set_pkm99("c_code", db.get_nextincrementlimitchar(code, 6));
                    }
                    else
                    {
                        db.DeleteOnTable("m02", "cmp_code='" + code + "'");
                        MessageBox.Show("Failed on saving.");
                    }
                }
                else
                {
                    col = "cmp_code='" + code + "', cmp_desc='" + cmp_desc + "', mag_code='" + mag_code + "'";

                    if (db.UpdateOnTable("m02", col, "cmp_code='" + code + "'"))
                    {
                        //success
                    }
                    else
        ...
        private void disp_dgvlist()
        {
            ...
            try
            {
                for (int r = 0; dt.Rows.Count > r; r++)
                {
                    ...
                    row.Cells["cmp_desc"].Value = dt.Rows[r]["cmp_desc"].ToString();
        ...
        private void disp_info()
        {
            try
            {
                ...
                txt_code.Text = dgv_list["cmp_code", r].Value.ToString();
                txt_cmp_desc.Text = dgv_list["cmp_desc", r].Value.ToString();
                cbo_mag_code.SelectedValue = dgv_list["mag_code", r].Value.ToString();
            }
            catch (Exception er) { MessageBox.Show(er.Message); }
        }
