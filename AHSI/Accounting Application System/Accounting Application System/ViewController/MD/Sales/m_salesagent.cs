...
{
    public partial class m_salesagent : Form
    {
        ...
        public m_salesagent()
        {
            ...
            DataTable dt2 = db2.QueryBySQLCode("SELECT a.*, b.* FROM rssys.x06 a LEFT JOIN rssys.x05 b ON a.mod_id = b.mod_id  WHERE a.grp_id = '" + grp_id + "' AND a.mod_id='M0407' ORDER BY b.pla, b.mod_id");
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (isnew)
                {
                    ...
                    {
                        success = false;
                        db.DeleteOnTable(table, "id='" + code + "'");
                        MessageBox.Show("Failed on saving.");
                    }
                }
                else
                {
                    col = "id='" + code + "', name='" + name + "'";

                    if (db.UpdateOnTable(table, col, "id='" + code + "'"))
                    {
                        success = true;
                    }
                    else
