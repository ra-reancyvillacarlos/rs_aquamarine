...
{
    public partial class m_stocklocation : Form
    {
        ...
        public m_stocklocation()
        {
            ...
            DataTable dt2 = db2.QueryBySQLCode("SELECT a.*, b.* FROM rssys.x06 a LEFT JOIN rssys.x05 b ON a.mod_id = b.mod_id  WHERE a.grp_id = '" + grp_id + "' AND a.mod_id='M0506' ORDER BY b.pla, b.mod_id");
        ...
        private void m_stocklocation_Load(object sender, EventArgs e)
        {
        ...
        private void btn_delitem_Click(object sender, EventArgs e)
        {
            ...
            if (dgv_list.Rows.Count > 1)
            {
                ...
                if (db.UpdateOnTable("whouse", "cancel='Y'", "whs_code='" + dgv_list["ID", r].Value.ToString() + "'"))
                {
                    disp_list();
                    goto_tbcntrl_list();
                    tpg_info_enable(false);
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select the branch of the location.");
            }
            else
            {
                code = txt_code.Text;
                loc_name = txt_name.Text;
                branch = cbo_branch.SelectedValue.ToString();

                if (isnew)
                {
                    code = db.get_pk("whs_code");
                    col = "whs_code, whs_desc, branch";
                    val = "'" + code + "', '" + loc_name + "', '" + branch + "'";

                    if (db.InsertOnTable("whouse", col, val))
                    {
                        success = true;
                        db.set_pkm99("whs_code", db.get_nextincrementlimitchar(code, 4));
                    }
                    else
                    {
                        success = false;
                        db.DeleteOnTable("whouse", "whs_code='" + code + "'");
                        MessageBox.Show("Failed on saving.");
                    }
                }
                else
                {
                    col = "whs_code='" + code + "', whs_desc='" + loc_name + "', branch='" + branch +"'";

                    if (db.UpdateOnTable("whouse", col, "whs_code='" + code + "'"))
                    {
                        success = true;
                    }
                    else
        ...
        private void disp_list()
        {
            ...
            try
            {
                for (int r = 0; dt.Rows.Count > r; r++)
                {
                    ...
                    row.Cells["id"].Value = dt.Rows[r]["whs_code"].ToString();
                    row.Cells["name"].Value = dt.Rows[r]["whs_desc"].ToString();
                    row.Cells["dgvl_branch"].Value = dt.Rows[r]["branchname"].ToString();
                    row.Cells["dgvl_branchid"].Value = dt.Rows[r]["branchcode"].ToString();
                }
            }
            catch (Exception) { }
        }
        ...
        private void disp_info()
        {
            try
            {
                ...
                branch = dgv_list["dgvl_branchid", r].Value.ToString();
