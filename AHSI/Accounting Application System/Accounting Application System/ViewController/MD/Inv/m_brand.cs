...
{
    public partial class m_brand : Form
    {
        ...
        public m_brand()
        {
            ...
            DataTable dt2 = db2.QueryBySQLCode("SELECT a.*, b.* FROM rssys.x06 a LEFT JOIN rssys.x05 b ON a.mod_id = b.mod_id  WHERE a.grp_id = '" + grp_id + "' AND a.mod_id='M0303' ORDER BY b.pla, b.mod_id");
        ...
        private void disp_list()
        {
            ...
            DataTable dt = db.get_brand_list();
            ...
            try
            {
                for (int r = 0; dt.Rows.Count > r; r++)
                {
                    ...
                    row.Cells["id"].Value = dt.Rows[r]["brd_code"].ToString();
                    row.Cells["name"].Value = dt.Rows[r]["brd_name"].ToString();
                }
            }
            catch (Exception) { }
        }
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (isnew)
                {
                    ...
                    col = "brd_code, brd_name";
                    val = "'" + code + "', '" + desc + "'";

                    if (db.InsertOnTable("brand", col, val))
                    {
                        success = true;
                    ...
                    {
                        success = false;
                        db.DeleteOnTable("brand", "brd_code='" + code + "'");
                        MessageBox.Show("Failed on saving.");
                    }
                }
                else
                {
                    col = "brd_code='" + code + "', brd_name='" + desc + "'";

                    if (db.UpdateOnTable("brand", col, "brd_code='" + code + "'"))
                    {
                        success = true;
                    }
                    else
