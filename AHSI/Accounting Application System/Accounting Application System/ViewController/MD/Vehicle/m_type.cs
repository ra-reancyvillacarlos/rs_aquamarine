...
{
    public partial class m_type : Form
    {
        ...
        private void m_type_Load(object sender, EventArgs e)
        {
        ...
        private void disp_list()
        {
            ...
            try
            {
                for (int r = 0; dt.Rows.Count > r; r++)
                {
                    ...
                    row.Cells["name"].Value = dt.Rows[r]["ctyp_desc"].ToString();
                }
            }
            catch (Exception) { }
        }
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Code must at least 3 characters long.");
            }
            else
            {
                code = txt_code.Text;
                ...
                if (isnew)
                {
                    ...
                    if (db.InsertOnTable("cartype", col, val))
                    {
                        success = true;
                    ...
                    {
                        success = false;
                        db.DeleteOnTable("cartype", "id='" + code + "'");
                        MessageBox.Show("Failed on saving.");
                    }
                }
                else
                {
                    col = "id='" + code + "', ctyp_desc='" + desc + "'";

                    if (db.UpdateOnTable("cartype", col, "id='" + code + "'"))
                    {
                        success = true;
                    }
                    else
