...
{
    public partial class m_workperform : Form
    {
        Boolean seltbp = false;
        Boolean isnew = false;

        public m_workperform()
        {
            InitializeComponent();
            disp_list();
        }

        private void m_workperform_Load(object sender, EventArgs e)
        {
        ...
        private void disp_list()
        {
            ...
            DataTable dt = db.QueryAllOnTable("service");
            ...
            try
            {
                for (int r = 0; dt.Rows.Count > r; r++)
                {
                    ...
                    row.Cells["id"].Value = dt.Rows[r]["srv_code"].ToString();
                    row.Cells["name"].Value = dt.Rows[r]["srv_name"].ToString();
                }
            }
            catch (Exception) { }
        }
        ...
        private void btn_delitem_Click(object sender, EventArgs e)
        {
            ...
            if (dgv_list.Rows.Count > 1)
            {
                ...
                if (db.UpdateOnTable("service", "cancel='Y'", "srv_code='" + dgv_list["ID", r].Value.ToString() + "'"))
                {
                    disp_list();
                    goto_tbcntrl_list();
                    tpg_info_enable(false);
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (isnew)
                {
                    ...
                    col = "srv_code, srv_name";
                    val = "'" + code + "', '" + desc + "'";

                    if (db.InsertOnTable("service", col, val))
                    {
                        success = true;
                    ...
                    {
                        success = false;
                        db.DeleteOnTable("service", "srv_code='" + code + "'");
                        MessageBox.Show("Failed on saving.");
                    }
                }
                else
                {
                    col = "srv_code='" + code + "', srv_name='" + desc + "'";

                    if (db.UpdateOnTable("service", col, "srv_code='" + code + "'"))
                    {
                        success = true;
                    }
                    else
