...
namespace Accounting_Application_System
{
    public partial class z_Notification : Form
    {
        ...
        public z_Notification()
        {
            ...
            tabPage3.Show();
            // End of by Roldan
            set_reslist(" WHERE arr_date = '"+DateTime.Now.ToString("yyyy-MM-dd")+"'");
        ...
        */
        ...
        public void set_reslist(String search_code)
        {
            ...
            try
            {
                DataTable dt_curr = db.QueryBySQLCode("SELECT res_code, arr_date, arr_time, full_name, pp.p_name, hl.name, COALESCE(SPLIT_PART(rf.occ_type, ', ', 4)) AS ttlpax, pck.package, pck1.activities, (CASE WHEN c_bool > 0 THEN true ELSE false END) AS cd, l_count AS lunch, res_date, reserv_by, remarks, arrived FROM rssys.resfil rf LEFT JOIN (SELECT name, code FROM rssys.hotel) hl ON hl.code = rf.hotel_code LEFT JOIN (SELECT DISTINCT string_agg(chg_desc, ', ') AS package, res_gfil.rg_code FROM (SELECT DISTINCT rg_code, chg_code FROM rssys.res_gfil) res_gfil LEFT JOIN (SELECT chg_code, chg_desc FROM rssys.charge) cgh ON res_gfil.chg_code = cgh.chg_code WHERE res_gfil.chg_code IN (SELECT chg_code FROM rssys.charge WHERE UPPER(chg_code) LIKE 'PCK%') GROUP BY res_gfil.rg_code) pck ON pck.rg_code = rf.res_code LEFT JOIN (SELECT string_agg(chg_desc, ', ') AS activities, (occ_type) AS ttlpax, rg_code FROM rssys.res_gfil LEFT JOIN (SELECT chg_code, chg_desc FROM rssys.charge) cgh ON res_gfil.chg_code = cgh.chg_code WHERE res_gfil.chg_code IN (SELECT chg_code FROM rssys.charge WHERE UPPER(chg_code) LIKE 'ACT%') AND UPPER(occ_type) LIKE '%ALL' GROUP BY rg_code, occ_type) pck1 ON pck1.rg_code = rf.res_code  LEFT JOIN (SELECT chg_desc AS p_name, chg_code FROM rssys.charge WHERE chg_type = 'P') pp ON pp.chg_code = p_typ LEFT JOIN (SELECT COUNT(rg.chg_code) AS l_count, rg_code FROM rssys.res_gfil rg WHERE rg.chg_code IN (SELECT chg_code FROM rssys.charge WHERE UPPER(chg_code) LIKE 'ADTL%' AND UPPER(chg_desc) LIKE 'LUNCH%') GROUP BY rg_code LIMIT 1) l_b ON l_b.rg_code = rf.res_code LEFT JOIN (SELECT COUNT(rg.chg_code) AS c_bool, rg_code FROM rssys.res_gfil rg WHERE rg.chg_code IN (SELECT chg_code FROM rssys.charge WHERE UPPER(chg_code) LIKE 'ADTL%' AND UPPER(chg_desc) LIKE 'CAMERA%') GROUP BY rg_code LIMIT 1) c_b ON c_b.rg_code = rf.res_code" + search_code + " ORDER BY arr_date, arr_time, hl.name ASC");
                DataTable dt_ld = db.QueryBySQLCode("SELECT COALESCE(SUM(COALESCE((CASE WHEN SPLIT_PART(REPLACE(rf.occ_type, ' ', ''), ',', 1) = '' THEN '0' ELSE SPLIT_PART(REPLACE(rf.occ_type, ' ', ''), ',', 1) END)::numeric(15,0), 0)), 0) AS adult, COALESCE(SUM(COALESCE((CASE WHEN SPLIT_PART(REPLACE(rf.occ_type, ' ', ''), ',', 2) = '' THEN '0' ELSE SPLIT_PART(REPLACE(rf.occ_type, ' ', ''), ',', 2) END)::numeric(15,0), 0)), 0) AS kid, COALESCE(SUM(COALESCE((CASE WHEN SPLIT_PART(REPLACE(rf.occ_type, ' ', ''), ',', 3) = '' THEN '0' ELSE SPLIT_PART(REPLACE(rf.occ_type, ' ', ''), ',', 3) END)::numeric(15,0), 0)), 0) AS inf, COALESCE(SUM(COALESCE((CASE WHEN SPLIT_PART(REPLACE(rf.occ_type, ' ', ''), ',', 4) = '' THEN '0' ELSE SPLIT_PART(REPLACE(rf.occ_type, ' ', ''), ',', 4) END)::numeric(15,0), 0)), 0) AS all FROM rssys.resfil rf WHERE arr_date = current_date");
                DataTable dt_chs = db.QueryBySQLCode("SELECT ch.chg_code, chg_desc, COALESCE(count, 0) AS count FROM rssys.charge ch LEFT JOIN (SELECT rg.chg_code, (CASE WHEN COUNT(rg.occ_type) > 1 THEN (MAX(COALESCE(SPLIT_PART((CASE WHEN rg.occ_type = '' THEN '0' ELSE rg.occ_type END), ' ', 1)::numeric, 0)) - MIN(COALESCE(SPLIT_PART((CASE WHEN rg.occ_type = '' THEN '0' ELSE rg.occ_type END), ' ', 1)::numeric, 0))) ELSE MAX(COALESCE(SPLIT_PART((CASE WHEN rg.occ_type = '' THEN '0' ELSE rg.occ_type END), ' ', 1)::numeric, 0)) END) AS count FROM rssys.res_gfil rg INNER JOIN rssys.resfil rf ON rg.rg_code = rf.res_code WHERE rf.arr_date = current_date GROUP BY rg.chg_code) cf ON cf.chg_code = ch.chg_code WHERE chg_type = 'C' AND (UPPER(ch.chg_code) LIKE 'PCK%' OR UPPER(ch.chg_code) LIKE 'ACT%') ORDER BY chg_desc ASC");

                dgv_reslist.DataSource = dt_curr;
                dataGridView1.DataSource = dt_chs;

                label2.Text = dt_curr.Rows.Count.ToString();
                label3.Text = dt_ld.Rows[0]["adult"].ToString().Replace(" ", "");
                label5.Text = dt_ld.Rows[0]["kid"].ToString().Replace(" ", "");
                label7.Text = dt_ld.Rows[0]["inf"].ToString().Replace(" ", "");
                label9.Text = ((dt_ld.Rows[0]["all"].ToString().Replace(" ", "") == "0") ? (Convert.ToDouble(dt_ld.Rows[0]["adult"].ToString().Replace(" ", "")) + Convert.ToDouble(dt_ld.Rows[0]["kid"].ToString().Replace(" ", "")) + Convert.ToDouble(dt_ld.Rows[0]["inf"].ToString().Replace(" ", ""))).ToString() : dt_ld.Rows[0]["all"].ToString().Replace(" ", ""));
