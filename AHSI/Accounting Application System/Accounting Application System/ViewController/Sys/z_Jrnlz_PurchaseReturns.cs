...
{
    public partial class z_Jrnlz_PurchaseReturns : Form
    {
        thisDatabase db;
        GlobalClass gc;
        ...
        public z_Jrnlz_PurchaseReturns()
        {
            InitializeComponent();
            ...
            gc.load_poinvoice_return_nonjrnlz(cbo_inv_frm);
            gc.load_poinvoice_return_nonjrnlz(cbo_inv_to);
        ...
        private void z_Jrnlz_PurchaseReturns_Load(object sender, EventArgs e)
        {
        ...
        private void bgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            dt = db.QueryOnTableWithParams("prethdr", "*", "pret_num between '" + inv_frm + "' and '" + inv_to + "' AND (jrnlz='N' OR jrnlz is null) AND cancel is null", "");
            ...
            for (i = 0; i < dtrows; i++)
            {
                inv_date = Convert.ToDateTime(dt.Rows[i]["trnx_date"].ToString()).ToString("yyyy-MM-dd");
                code = dt.Rows[i]["pret_num"].ToString();
                ...
                {
                    dt2 = db.QueryOnTableWithParams("pretlne", "*", "pret_num='" + code + "'", "");
                    ...
                    for (j = 0; j < dt2.Rows.Count; j++)
                    {
                        ...
                        item_arr.price = dt2.Rows[j]["price"].ToString();
                        item_arr.recv_qty = dt2.Rows[j]["ret_qty"].ToString();
                    ...
                    if (pay_code != "COD")
                        db.InsertOnTable("tr06", "c_code, invoice, t_date, j_code, j_num, seq_desc, debit, credit",
                            "'" + supl_code + "', '" + invoice + "', '" + inv_date + "', '" + j_code + "', '" + j_num + "', " + db.str_E(t_desc) + ", " + total_amt.ToString("0.00") + ", " + (total_amt*-1).ToString("0.00"));

                    noOfjrnl++;

                    inc_pbar(inc);

                    db.UpdateOnTable("prethdr", "jrnlz='Y'", "pret_num='" + code + "'");
