...
{
    public partial class market_segment : Form
    {
        ...
        private void m_salesagent_Load(object sender, EventArgs e)
        {
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            String code, name, pwd, branch = db.get_m99branch();
            ...
            else
            {
                ...
                if (isnew)
                {
                    ...
                    col = "mkt_code, mkt_desc,branch";
                    val = "'" + code + "', '" + name + "', '" + branch + "'";
                    ...
                    {
                        success = false;
                        db.DeleteOnTable(table, "mkt_code='" + code + "'");
                        MessageBox.Show("Failed on saving.");
                    }
                }
                else
                {
                    col = "mkt_code='" + code + "', mkt_desc='" + name + "'";

                    if (db.UpdateOnTable(table, col, "mkt_code='" + code + "'"))
                    {
                        success = true;
                    }
                    else
        ...
        {
            thisDatabase db = new thisDatabase();
            DataTable dt = db.get_market_list();
            ...
            try
            {
                for (int r = 0; dt.Rows.Count > r; r++)
                {
                    ...
                    row.Cells["id"].Value = dt.Rows[r]["mkt_code"].ToString();
                    row.Cells["name"].Value = dt.Rows[r]["mkt_desc"].ToString();
                }
            }
            catch (Exception) { }
        }
