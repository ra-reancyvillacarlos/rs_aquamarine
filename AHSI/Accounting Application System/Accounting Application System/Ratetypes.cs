...
{
    public partial class Ratetypes : Form
    {
        ...
        private void disp_list()
        {
            ...
            DataTable dt = db.QueryBySQLCode("SELECT rate_code, rate_desc, active, rmrttyp from rssys.ratetype ORDER BY rate_code ASC");
            ...
            try
            {
                for (int r = 0; dt.Rows.Count > r; r++)
                {
                    ...
                    row.Cells["id"].Value = dt.Rows[r]["rate_code"].ToString();
                    row.Cells["name"].Value = dt.Rows[r]["rate_desc"].ToString();
                }
            }
            catch (Exception) { }
        }
        ...
        private void btn_delitem_Click(object sender, EventArgs e)
        {
            ...
            if (dgv_list.Rows.Count > 1)
            {
                ...
                if (db.UpdateOnTable("ratetype", "cancel='Y'", "brd_code='" + dgv_list["ID", r].Value.ToString() + "'"))
                {
                    disp_list();
                    goto_tbcntrl_list();
                    tpg_info_enable(false);
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (isnew)
                {
                    ...
                    col = "rate_code, rate_desc";
                    val = "'" + code + "', '" + desc + "'";

                    if (db.InsertOnTable("ratetype", col, val))
                    {
                        success = true;
                    ...
                    {
                        success = false;
                        db.DeleteOnTable("ratetype", "rate_code='" + code + "'");
                        MessageBox.Show("Failed on saving.");
                    }
                }
                else
                {
                    col = "rate_code='" + code + "', rate_desc='" + desc + "'";

                    if (db.UpdateOnTable("ratetype", col, "rate_code='" + code + "'"))
                    {
                        success = true;
                    }
                    else
