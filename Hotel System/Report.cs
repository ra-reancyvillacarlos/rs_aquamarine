...
namespace Hotel_System
{
    public partial class Report : Form
    {
        ...
        public void print_monthlyreserv(String __month)
        {
            ...
            //trans_no = _soa_code;
            sqqq = "SELECT tr.trv_name AS tenant, SUM(COALESCE(CASE WHEN ((CASE WHEN REPLACE(SPLIT_PART(occ_type, ', ', 1), ' ', '') = '' THEN '0' ELSE REPLACE(SPLIT_PART(occ_type, ', ', 1), ' ', '') END)::numeric(15,0) + (CASE WHEN REPLACE(SPLIT_PART(occ_type, ', ', 2), ' ', '') = '' THEN '0' ELSE REPLACE(SPLIT_PART(occ_type, ', ', 2), ' ', '') END)::numeric(15,0) + (CASE WHEN REPLACE(SPLIT_PART(occ_type, ', ', 3), ' ', '') = '' THEN '0' ELSE REPLACE(SPLIT_PART(occ_type, ', ', 3), ' ', '') END)::numeric(15,0)) > (CASE WHEN REPLACE(SPLIT_PART(occ_type, ', ', 4), ' ', '') = '' THEN '0' ELSE REPLACE(SPLIT_PART(occ_type, ', ', 4), ' ', '') END)::numeric(15,0) THEN ((CASE WHEN REPLACE(SPLIT_PART(occ_type, ', ', 1), ' ', '') = '' THEN '0' ELSE REPLACE(SPLIT_PART(occ_type, ', ', 1), ' ', '') END)::numeric(15,0) + (CASE WHEN REPLACE(SPLIT_PART(occ_type, ', ', 2), ' ', '') = '' THEN '0' ELSE REPLACE(SPLIT_PART(occ_type, ', ', 2), ' ', '') END)::numeric(15,0) + (CASE WHEN REPLACE(SPLIT_PART(occ_type, ', ', 3), ' ', '') = '' THEN '0' ELSE REPLACE(SPLIT_PART(occ_type, ', ', 3), ' ', '') END)::numeric(15,0)) ELSE (CASE WHEN REPLACE(SPLIT_PART(occ_type, ', ', 4), ' ', '') = '' THEN '0' ELSE REPLACE(SPLIT_PART(occ_type, ', ', 4), ' ', '') END)::numeric(15,0) END, '0')::numeric(15,0)) AS others, SUM(COALESCE(cf.amount, 0.00)) AS amount FROM rssys.travagnt tr LEFT JOIN (SELECT * FROM rssys.gfolio WHERE (t_date BETWEEN '" + DateTime.Now.Year + "-" + __month__.ToString("00") + "-01' AND '" + t_date + "')) gf ON tr.trv_code = gf.trv_code LEFT JOIN (SELECT cf.* FROM rssys.chgfil cf INNER JOIN rssys.charge ch ON ch.chg_code = cf.chg_code WHERE ch.chg_type = 'C') cf ON gf.reg_num = cf.reg_num GROUP BY tr.trv_name ORDER BY tr.trv_name ASC";
        ...
        private void bgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    inc_pbar(4);
                    DataTable dt1231 = db.QueryBySQLCode(sqqq), dt2 = new DataTable();
                    inc_pbar(40);
                    myReportDocument.Load(fileloc_acctg + "CrystalReport1.rpt");
                    myReportDocument.Database.Tables[0].SetDataSource(dt1231);
                    inc_pbar(20);
                    //clr_param();
