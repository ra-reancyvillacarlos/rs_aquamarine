...
namespace Hotel_System
{
    public partial class newGuestBilling : Form
    {
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
            ...
            {
                thisDatabase db = new thisDatabase();
                String soa_code = dgv_gfolio.CurrentRow.Cells[8].Value.ToString();
                ...
                String chg_num = dgv_gfolio.CurrentRow.Cells[14].Value.ToString();

                //MessageBox.Show(soa_code + " " + chg_code + " " + db.get_colval("m99", "fnlz_chg", ""));
                if(chg_code == db.get_colval("m99", "fnlz_chg", ""))
                {
                    MessageBox.Show("Selected charge cannot update.");
                }
                else if (String.IsNullOrEmpty(soa_code))
                {
                    if (String.IsNullOrEmpty(trnxdt) && String.IsNullOrEmpty(doctyp) && String.IsNullOrEmpty(refer) && String.IsNullOrEmpty(amt) && String.IsNullOrEmpty(resnum) && String.IsNullOrEmpty(res_gname) && String.IsNullOrEmpty(rm) && String.IsNullOrEmpty(chg_num))
                    {
                        MessageBox.Show("Pls select folio");
                    }
                    else
                ...
                {
                    MessageBox.Show("Selected charge already finalized.");
                }
            }
            else
            {
        ...
        private void dis_dgvguest(String cond)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    row.Cells[9].Value = db.get_colval("m04", "at_desc", "at_code='" + db.get_colval("m06", "at_code", "d_code='" + dt.Rows[i][11].ToString() + "'") + "'");
        ...
        public void disp_chgfil(String reg_num)
        {
            ...
            try
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    dgv_row.Cells[8].Value = row[13].ToString();

                    dgv_row.Cells[9].Value = row[7].ToString();
                    dgv_row.Cells[10].Value = row[8].ToString();
                    dgv_row.Cells[11].Value = row[9].ToString();
                    dgv_row.Cells[12].Value = row[10].ToString();
                    dgv_row.Cells[13].Value = row[11].ToString();
                    dgv_row.Cells[14].Value = row[12].ToString();
                    dgv_row.Cells[15].Value = row[15].ToString();
                ...
                for (int i = dgv_gfolio.Rows.Count - 1; i > -1 ; i--)
                {
                    chg_code = (dgv_gfolio[1, i].Value??"").ToString();
                    if (!String.IsNullOrEmpty(chg_code))
                    {
                        chg_amt = dgv_gfolio[5, i].Value.ToString();
                        if (!db.is_chg_deposit(chg_code))
                        {
                            chg_amt_no_deposit = chg_amt;
                            runbal = runbal + (gm.toNormalDoubleFormat(chg_amt_no_deposit));
                            dgv_gfolio[6, i].Value = gm.toAccountingFormat(runbal);
                        }
                    }
                }
            }
            ...
            bal_amnt = db.get_guest_charges_total(reg_num, true);
        ...
        private void disp_guestinfo(String reg_num)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                Double night = 0, mo = 0, wk = 0;
                night = Convert.ToInt32((Convert.ToDateTime(lbl_depdate.Text) - Convert.ToDateTime(lbl_arrdate.Text)).TotalDays); 
                ...
                if (rmrttyp == "M")
                {
                    ...
                    label9.Text = "No. of Month(s)";
                    lbl_noofnights.Text = mo.ToString("0");
                }
                else if (rmrttyp == "W")
                {
                    ...
                    label9.Text = "No. of Week(s)";
                    lbl_noofnights.Text = wk.ToString("0");
                }
                else
                {
                    label9.Text = "No. of Nights";
                    lbl_noofnights.Text = night.ToString("0");
                }
